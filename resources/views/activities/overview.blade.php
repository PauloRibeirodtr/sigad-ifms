...
@section('page_subtitle', 'Indicadores por Plano de Trabalho')
    ...
    <section class="grid gap-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <h2 class="mt-1 text-2xl font-extrabold tracking-tight text-slate-950">Atividades por Plano de Trabalho</h2>
            ...
            <form method="GET" action="{{ route('activities.overview') }}" class="flex w-full max-w-xl flex-col gap-2 sm:flex-row">
                ...
                <label for="busca_plano" class="sr-only">Buscar Plano de Trabalho</label>
                <input id="busca_plano" name="busca" value="{{ request('busca') }}" placeholder="Buscar Plano de Trabalho" class="min-h-12 min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-4 text-sm shadow-sm focus:border-brand-500 focus:ring-4 focus:ring-brand-100">
        ...
        <nav class="flex gap-2 overflow-x-auto pb-1" aria-label="Indicadores de atividades">
            <a href="{{ route('activities.overview', request()->only('busca')) }}" class="whitespace-nowrap rounded-full px-4 py-2 text-sm font-bold {{ $indicator === null ? 'bg-brand-700 text-white' : 'border border-slate-200 bg-white text-slate-600' }}">Todos os planos</a>
        ...
        @if ($indicator)
            <x-alert type="info">Exibindo somente Planos de Trabalho com atividades classificadas como <strong>{{ mb_strtolower($indicator->label()) }}</strong>.</x-alert>
        @endif

        @if ($plans->isEmpty())
            <div class="rounded-3xl border border-dashed border-brand-300 bg-white px-6 py-16 text-center shadow-card">
                <h3 class="text-lg font-extrabold text-slate-900">Nenhum Plano de Trabalho encontrado</h3>
                <p class="mt-2 text-sm text-slate-500">Não há planos correspondentes à busca ou ao indicador selecionado.</p>
        ...
        @else
            <p class="text-sm font-semibold text-slate-500">{{ $plans->total() }} {{ $plans->total() === 1 ? 'plano encontrado' : 'planos encontrados' }}</p>
