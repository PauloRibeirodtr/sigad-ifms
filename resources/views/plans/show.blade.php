...
@section('page_title', 'PAT')
@section('page_subtitle', 'Plano de atividades vinculado ao PIT '.$plan->pit->nome)

@section('content')
    <div class="grid gap-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <a href="{{ route('pits.show', $plan->pit) }}" class="inline-flex items-center gap-2 text-sm font-bold text-brand-700 transition hover:text-brand-900"><span aria-hidden="true">←</span> Voltar para o PIT {{ $plan->pit->nome }}</a>
            <a href="{{ route('plans.edit', $plan) }}" class="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-200 bg-white px-5 text-sm font-bold text-slate-700 transition hover:bg-slate-100">Editar PAT</a>
        </div>
        ...
        <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-card">
            <div class="bg-brand-800 px-6 py-7 text-white sm:px-8">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        <p class="text-sm font-bold text-brand-100">PAT #{{ $plan->id }} · PIT {{ $plan->pit->nome }}</p>
            ...
            <div class="grid gap-7 p-6 sm:p-8">
                ...
                <dl class="grid gap-4 border-y border-slate-100 py-6 sm:grid-cols-3">
                    <div class="rounded-2xl bg-slate-50 p-4"><dt class="text-xs font-bold text-slate-500">Início do PIT</dt><dd class="mt-2 text-base font-extrabold text-slate-900">{{ $plan->data_inicial->format('d/m/Y') }}</dd></div>
                    <div class="rounded-2xl bg-slate-50 p-4"><dt class="text-xs font-bold text-slate-500">Fim do PIT</dt><dd class="mt-2 text-base font-extrabold text-slate-900">{{ $plan->data_final->format('d/m/Y') }}</dd></div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 px-5 py-6">
                    <h3 class="font-extrabold text-slate-900">Atividades do PAT</h3>
